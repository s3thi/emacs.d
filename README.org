...
use-package. We load it early so other packages can use =:diminish=.
...
the first part of a long keychord. Note that =which-key= is built into
Emacs 30+, so we only install it on older versions.
...
=prettier-js= formats code using Prettier on save.
...
Unified setup for JS, TS, and Astro modes. This ensures node_modules is added to =exec-path= before LSP and Prettier start.
